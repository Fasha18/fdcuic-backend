...
const AdminNotifications = () => {
  ...
  const columns = [
    { key: 'destinataire', label: 'Destinataire', render: (n) => <span style={{ fontWeight: 600 }}>{n.destinataire ? `${n.destinataire.prenom} ${n.destinataire.nom}` : 'Système/Global'}</span> },
    { key: 'message', label: 'Message', render: (n) => <div style={{ maxWidth: 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={n.message}>{n.message}</div> },
    ...
    { key: 'statut', label: 'Statut', render: (n) => (
      <span style={{ 
        padding: '4px 10px', borderRadius: '20px', fontSize: 12, fontWeight: 700,
    ...
    { key: 'date', label: 'Date d\'envoi', render: (n) => new Date(n.date_envoi).toLocaleString('fr-FR') }
  ];
