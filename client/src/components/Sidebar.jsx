...
const Sidebar = ({ activeTab, onTabChange, onLogout, role = 'admin' }) => {
  ...
  const handleNavigation = (id) => {
    ...
    // Fallback navigation in case onTabChange doesn't cover all cases (like in some candidate pages)
    const routeMapCandidat = {
      'apercu': '/candidat',
      'opportunites': '/candidat/appels',
      'mes-candidatures': '/candidat/mes-dossiers',
      'mobilite': '/candidat/mobilite',
      'ressources': '/candidat/ressources',
      'parametres': '/candidat/parametres',
      'profil': '/candidat/profil'
    };
    const routeMapAdmin = {
      'apercu': '/admin',
      'campagnes': '/admin/appels',
      'mobilite': '/admin/mobilite',
      'types-projet': '/admin/types-projet',
    ...
    const map = role === 'candidat' ? routeMapCandidat : routeMapAdmin;
    if (map[id]) {
      navigate(map[id]);
    }
  };
  ...
  const navGroups = role === 'admin'
    ? NAV_ADMIN
    : role === 'evaluateur'
    ? [{ section: 'Menu', items: NAV_ADMIN[0].items.filter(item => !ITEMS_EVALUATEUR_BLOQUES.includes(item.id)) }]
    : NAV_CANDIDAT;
    ...
    <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`} style={{
      ...
      <div className="sidebar-header" style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, overflow: 'hidden' }}>
          <img src="/FDCUIC_logo.png" alt="FDCUIC Logo" style={{ width: isCollapsed ? 50 : 130, height: 'auto', flexShrink: 0 }} />
          {!isCollapsed && (
            <div style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 11, color: 'var(--color-text-secondary)', lineHeight: 1.4, whiteSpace: 'normal', paddingRight: 8, fontWeight: 500, letterSpacing: '0.2px' }}>
                Fonds de Développement des Cultures Urbaines et des Industries Créatives
              </div>
              <div style={{ fontSize: 11, color: 'var(--color-primary)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                {role === 'admin' ? 'Administration' : role === 'evaluateur' ? 'Espace Évaluateur' : 'Espace Candidat'}
              </div>
            </div>
          )}
        </div>
      ...
      <div ref={dropdownRef} style={{
        ...
        {isDropdownOpen && (
          <div style={{
            ...
            {[
              { id: 'profil', label: 'Mon Profil', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
              { id: 'parametres', label: 'Paramètres', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg> },
            ].map(item => (
              <button 
                key={item.label} 
                onClick={() => handleNavigation(item.id)}
